...
public class NetworkTest {
	...
	public void setUpBefore() throws Exception {
		...
		for (int index = 0; index < COUNT; index++) {
			Connection conn = new Connection(new OutputStreamWriter(System.out),
		...
		for (int nodeIndex = 0; nodeIndex < COUNT; nodeIndex++) {
			for (int other = nodeIndex + 1; other < COUNT; other++) {
				if (rand.nextFloat() <= chance) {
					...
					if (conn.getGroup().equals(connOther.getGroup()))
						linkList.addElement(new Link(conn.getNode(), connOther.getNode(), group));
				}
			}
		}
	...
	public void testReturnOffset() {
		for (int index = 0; index < COUNT; index++) {
			...
			for (int distance = 1; distance < 4; distance++) {
				...
				assertEquals("out and back (" + distance + ") from " + conn.getNode(), conn.getNode(), network.offsetNode(next, -distance));
			}
		}
	}
