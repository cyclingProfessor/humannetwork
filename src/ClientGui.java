...
public class ClientGui extends JFrame {
  ...
  private enum BTN_DETAILS {
    SEND(posn, "Send",
        "Press to send the current message to someone on the network",
        1), ADD(posn, "Add",
            "Press to add the message content to the list of messages",
            2), MOVE_UP(posn, "Move Up",
                "Press to make the currently selected message move up in the list",
                1), MOVE_DOWN(posn, "Move Down",
                    "Press to make the currently selected message move down in the list",
                    2), MERGE(posn, "Combine",
                        "Press to merge the selected messages", 1), SPLIT(posn,
                            "Split",
                            "Press to split the message into small chunks",
                            2), ADD_CHECK(posn, "Add Checksum",
                                "Press to add a checksum at the end of the current messsage",
                                1), VERIFY(posn, "Verify Checksum",
                                    "Press to verify if the checksum of the current message is correct",
                                    1), ENCRYPT(posn, "Encrypt",
                                        "Press to encrypt the current message",
                                        1), DECRYPT(posn, "Decrypt",
                                            "Press to decrypt the current message",
                                            1), NONCE(posn, "Add Random Number",
                                                "Press to add a random number to the current message",
                                                2), DELETE(posn, "Delete",
                                                    "Press to delete the selected messages",
                                                    1);
  ...
  private void setSizes() {
    ...
    setRelativeBounds(scrollPane, 0, LIST_TOP, 1.0f,
        BTN_DETAILS.DELETE.offset() + BTN_HEIGHT - LIST_TOP);
  }
  ...
  private void setButtonBounds(JButton b, float y) {
    ...
    b.setBounds(left, Math.round(h_factor * y),
        Math.round(h_factor * BTN_WIDTH), Math.round(h_factor * BTN_HEIGHT));
  }
  ...
  private void setRelativeBounds(Component c, float x, float y, float width,
    ...
    c.setBounds(Math.round(h_factor * BORDER + w_factor * x),
        Math.round(h_factor * y), Math.round(w_factor * width),
        Math.round(h_factor * height));
  }

  private void setStatusBounds(Component c) {
    float h_factor = 1.14f * getHeight();
    int border = Math.round(h_factor * STATUS_TOP);
    c.setBounds(border, border, getWidth() - 3 * border,
        Math.round(h_factor * STATUS_HEIGHT));
  }
